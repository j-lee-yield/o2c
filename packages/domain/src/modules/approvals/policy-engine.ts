...
export class NoopApprovalPolicyEngine implements ApprovalPolicyEngine {
  evaluateTransition(_context: ApprovalPolicyContext): ApprovalPolicyDecision {
    return { allowed: true, reasons: [] };
  }
}

export class RoleAwareApprovalPolicyEngine implements ApprovalPolicyEngine {
  evaluateTransition(context: ApprovalPolicyContext): ApprovalPolicyDecision {
    if (
      (context.nextStatus === "approved" || context.nextStatus === "rejected") &&
...
export type ApprovalPolicyAction =
  ...
  | "discount_settlement_writeoff_message"
  | "grouped_reminder"
  | "cash_auto_apply"
  | "erp_writeback_override";
...
export function evaluateApprovalRequirement(
  input: ApprovalRequirementContext
  ...
  if (
    input.subject === "cash_application" &&
...
function principalCanReviewAssignedApproval(principal: Principal, assigneeRole: Role) {
  return principal.roles.includes("admin") || principal.roles.includes(assigneeRole);
}

function approvalRequestTypeFor(input: ApprovalRequirementContext) {
...
function buildAutoExecutionSummary(input: ApprovalRequirementContext) {
  switch (input.action) {
    case "invoice_resend":
...
function canAutoExecuteWithoutApproval(input: ApprovalRequirementContext) {
  if (input.subject === "cash_application") {
    return Boolean(
      input.action === "cash_auto_apply" &&
  ...
  if (input.action === "broken_promise_follow_up") {
    return Boolean(
      input.verifiedContact && input.accountTier !== "strategic" && !input.hasDisputedInvoice
    );
  }
...
function buildPolicyContext(input: ApprovalRequirementContext): Record<string, unknown> {
  return {
